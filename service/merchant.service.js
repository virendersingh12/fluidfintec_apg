...
const createNewPendingUser = async (data, user_password, roles) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const user = await Merchant.findOne({ email: data.email, isActivated: true });
...
// resend activation email and create new token
const resendActivationEmailService = async (email) => {
  ...
  // Send email
  await emailService.sendMerchantRegistrationEmail(
    merchant.email,
