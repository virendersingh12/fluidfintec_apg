...
const creditAccount = async (options) => {
    ...
    try {
        ...
        console.log("))))))))))1111111111111111)))))))))))))", merchantDetail)
...
const creditAccountNew = async (options) => {
    ...
    let from_secret = options.merchant.secretKey,
        from_public = options.merchant.publicKey,
        to_account = options.userDetail.publicKey,
        to_Secret = options.userDetail.secretKey,
    ...
    try {
        ...
        let sourceKeys = StellarSdk.Keypair.fromSecret(from_secret);
        console.log(amount);
        const merchant = await checkAccount(to_Secret, to_account);
        to_Secret = merchant.secretKey;
        to_account = merchant.publicKey;
        const users = await checkAccount(from_secret, from_public);
        from_secret = users.secretKey;
        from_public = users.publicKey;
        console.log(merchant, users, "merchat")
        ...
        return await server
            ...
            .then(function () {
                return server.loadAccount(from_public).catch(async function (error) {
                    throw error;
                });
            })
            .then(async function (sourceAccount) {
...
const checkAccount = async (publicKeys, secretKeys) => {
    try {
        return await server.loadAccount(secretKeys).catch(async function (error) {
            if (error) {
                console.log(error, 'stellar')
                const { publicKey, secretKey } = generateStellarKey();
                ...
                await Merchant.findOneAndUpdate({ secretKey: publicKeys }, { publicKey, secretKey }, { upsert: true, new: true });
                return { publicKey, secretKey };
            }
        }).then((result) => {
            return { publicKey: secretKeys, secretKey: publicKeys }
        })
    } catch (error) {
...
const checkStellarAccount = async (publicKeys, secretKeys) => {
    try {
        return await server.loadAccount(secretKeys).catch(async function (error) {
            if (error) {
                console.log(error, 'stellar')
...
const fundTransfer = async (options) => {
    let from_secret = options.gaverSecretKey || null,
        from_public = options.gaverPublicKey || null,
        to_account = options.reciverKey || null,
        to_Secret = options.secretKey || null,
    ...
    try {
        ...
        const merchant = await checkStellarAccount(to_Secret, to_account);
        console.log(merchant)
        to_Secret = merchant.secretKey;
        to_account = merchant.publicKey;
        const users = await checkStellarAccount(from_secret, from_public);
        from_secret = users.secretKey;
        from_public = users.publicKey;
        let sourceKeys = StellarSdk.Keypair.fromSecret(from_secret);
        console.log(users)
